class User < ActiveRecord::Base
  ...
  def self.top_rated
    self.select('users.*') #Select all attributes of the user
        ...
        .select('COUNT(DISTINCT comments.id) + COUNT(DISTINCT posts.id) AS rank') # ADD the comment count to the post
        ...
        .order('rank DESC') # Instrucst the database to order the results in descending order, by the rank that we created in this query. ( rank = comment count + post count)
  end
end
